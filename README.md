...
- **헤드라이트**: Y 버튼을 200ms 이상 누르고 뗄 때 ON/OFF 토글
- **볼륨 조절**: L1 버튼으로 감소, R1 버튼으로 증가 (100ms 간격)
...
- **Y 버튼 길게(≥200ms) 후 릴리스**: 헤드라이트 ON/OFF 토글
...
### 볼륨 조절
- **L1 버튼**: 볼륨 감소 (11-30 범위, 100ms 간격)
- **R1 버튼**: 볼륨 증가 (11-30 범위, 100ms 간격)
...
| **L1 버튼** | 볼륨 감소 | 11-30 범위 |
| **R1 버튼** | 볼륨 증가 | 11-30 범위 |
| **Y 버튼 길게(≥200ms) 후 릴리스** | 헤드라이트 토글 | ON/OFF |
